...
try {
    ...
    if ($method === 'POST' && $action === 'register') {
        ...
        try {
            ...
            $verificationRequired = (bool) config('EMAIL.verification_enabled', false);
            $emailWarning = null;
            ...
            if ($verificationRequired) {
                ...
                // Email is an optional service in local development.  A missing
                // local mail transport must never roll back a successful signup.
                if (!$mailer->sendVerificationEmail($email, $token, $firstName . ' ' . $lastName)) {
                    $emailWarning = 'Account created, but the verification email could not be sent. Configure email settings before enabling verification emails.';
                }
            }

            $notificationService = new NotificationService();
            if (!$notificationService->sendWelcomeEmail($email, $firstName, $role)) {
                $emailWarning = $emailWarning ?: 'Account created, but the welcome email could not be sent.';
            }
