...
{
    if (!config('SECURITY.enable_cors', false)) {
        return;
    }
...
{
    \App\Middleware\RateLimitMiddleware::enforce($identifier, $endpoint, $limit);
}
...
try {
    ...
    if ($method === 'POST' && $action === 'login') {
        ...
        if (config('DEMO.seed_users', false)) {
        ...
        $sessionUser = (new \App\Services\AuthService($userRepo))->createSession($user);
        ...
        apiResponse(true, 'Login successful.', [
            'role' => $effectiveRole,
            'user' => $sessionUser,
            'dashboard' => buildDashboardData($pdo, $sessionUser),
    ...
    if ($method === 'POST' && $action === 'submit-assignment') {
        ...
        try {
            ...
            if (!Validator::validDeadline((string) ($assignment['due_date'] ?? ''))) {
                apiResponse(false, 'The assignment deadline has passed.', null, 422);
            }
